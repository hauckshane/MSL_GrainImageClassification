...
```{r, echo=FALSE, fig.align='center', fig.cap='Cross-Validation Accuracy for each model.', message=FALSE, warning=FALSE}
library(knitr)
...
) %>% as.data.frame() %>% mutate(Accuracy = round(Accuracy, 4))

# Print the accuracy scores ordered by accuracy
kable(accuracy_scores %>% arrange(desc(Accuracy)), caption = "Initial Cross-Validation Accuracy Scores for Each Model.")
```

```{r, echo=FALSE, fig.align='center', fig.cap='Bootstrapped accuracy scores for each model.'}
...
)  %>% as.data.frame() %>% mutate(Boot_Accuracy = round(Boot_Accuracy, 4))

# Print the bootstrapped accuracy scores ordered by accuracy
kable(boot_accuracy_scores %>% arrange(desc(Boot_Accuracy)), caption = "Bootstrapped Accuracy Scores for Each Model.")

```

When examining Table 1 and Table 2, we can see that all of our selected methods returned very high accuracies for both initial cross-validation and bootstrapped cross-validation. SVM was the top performer for during initial cross-validation. After bootstrapping, we found that LASSO, Elastic Net, and SVM consistently performed best on the given training data. We do not have complete confidence that any of these methods are superior to the rest from looking at the ridge plots in Figure 4. Aside from QDA and KNN, all of the ridges look generally in line with each other in terms of accuracy and even range. The highest performers barely edge ahead and there should absolutely be caution taken when applying these methods on unseen data.

From our analysis, we decided that SVM was the best classifier. This made sense as our PCA was displaying potential separability that was not directly linear in a two dimensional space. We used a radial kernel for the SVM as opposed to a linear kernel and doing this gave us consistently good results. To improve the model further, we also found the ideal tuning parameters to give the best results. Essentially, we iteratively built SVMs with different combinations of cost and gamma values and found a model with tuning parameters that consistently returned the highest accuracy on test sets. The ideal cost is 0.7 and the ideal gamma is 0.01.

```{r, echo=FALSE, out.width="75%", fig.align='center', fig.cap='Ridge plots show the distributions of bootstrapped accuracy scores.', warning=FALSE, message=FALSE}
# RBind all Bootstrapped dfs
rbind(data.frame(Variable = "Lasso", "Accuracy" = boot_lasso_acc),
      data.frame(Variable = "Ridge", "Accuracy" = boot_ridge_acc),
      data.frame(Variable = "Elastic Net", "Accuracy" = boot_enet_acc),
      data.frame(Variable = "Random Forest", "Accuracy" = boot_rf_acc),
      data.frame(Variable = "Gradient Boosting", "Accuracy" = boot_xgb_acc),
      data.frame(Variable = "K-Nearest Neighbors", "Accuracy" = boot_knn_acc),
      data.frame(Variable = "Support Vector Machine", "Accuracy" = boot_svm_acc),
      data.frame(Variable = "Tree", "Accuracy" = boot_tree_acc),
      data.frame(Variable = "Linear Discriminant Analysis", "Accuracy" = boot_lda_acc),
      data.frame(Variable = "Quadratic Discriminant Analysis", "Accuracy" = boot_qda_acc)) %>%
    suppressMessages() -> boot_df
...
boot_df %>%
  group_by(Variable) %>%
  mutate(meanAcc = mean(Accuracy)) %>%
  ggplot(aes(x = Accuracy, y = reorder(Variable, meanAcc))) +
  geom_density_ridges(scale = 1, rel_min_height = 0.01, ) +
  labs(title = "Bootstrapped Accuracy Scores for Each Model",
       x = "Accuracy",
       y = "Model") +
  theme_minimal()
```
...
The resulting SVM has an F1 score of 0.9367 and consistent accuracy of approximately 92.2% from bootstrapping cross-validation. When looking at the confusion matrix of the SVM (Figure 5), we can see that the model is performing really well and the number of false positives and negatives looks roughly even. The ROC curve for the model (Figure 6) also looks ideal. There is an immediate spike in sensitivity as specificity increases meaning that the model performs extremely better than a random classifier. This also accounts for the intricacies of the data’s variability at higher dimensions that requires a very complex decision boundary that can be achieved with SVMs.

If we were given more time on this project, there are several different steps we could take to improve our results. We could methodically transform the current sixteen features to get more complex features that could help the SVM deal with complexity. We could also try more complex classifiers such as training a neural network with regards to the intertwined variability of the data. A study such as this would benefit from more data as well so that we can perform more rigorous feature selection. With more careful analysis and the right amount of computing power, there are definitely several paths of improvement that can be taken despite our optimistic results.
...
# Print the bootstrapped accuracy scores ordered by accuracy
kable(boot_accuracy_scores %>% arrange(desc(Boot_Accuracy)), caption = "Bootstrapped Accuracy Scores for Aach Model.")
```
